...
let cbN=0,loadingSeg=false,loadingInc=false;
...
function renderSegnalazioni(){const all=[...segnalazioni].sort((a,b)=>segTimestamp(b.Data)-segTimestamp(a.Data));const counts={all:all.length,'Aperta':all.filter(r=>(r.Stato||'Aperta')==='Aperta').length,'In gestione':all.filter(r=>r.Stato==='In gestione').length,'Chiusa':all.filter(r=>r.Stato==='Chiusa').length};$('stat-total').textContent=counts.all;$('stat-aperte').textContent=counts.Aperta;$('stat-gestione').textContent=counts['In gestione'];$('stat-chiuse').textContent=counts.Chiusa;const rows=currentSegFilter==='all'?all:all.filter(r=>(r.Stato||'Aperta')===currentSegFilter);if(!all.length){$('table-container').innerHTML=emptyState('report','Nessuna segnalazione','Le nuove segnalazioni appariranno qui.');return}if(!rows.length){$('table-container').innerHTML=emptyState('report','Nessuna segnalazione in questa categoria','Scegli un altro filtro per vedere le altre segnalazioni.');return}
 const desktop=`<div class="seg-desktop-table"><table aria-label="Segnalazioni ricevute"><thead><tr><th scope="col" style="width:18%">Data</th><th scope="col" style="width:20%">Componente</th><th scope="col" style="width:24%">Contatto</th><th scope="col" style="width:24%">Stato</th><th scope="col" style="width:14%">Risposta</th></tr></thead><tbody>${rows.map(r=>{const i=segnalazioni.indexOf(r),url=safeExternalUrl(r['Link Risposta']);return `<tr><td>${escapeHtml(dateLong(r.Data))}</td><td><span class="badge ${componentClass(r.Componente)}">${escapeHtml(componentDisplay(r.Componente))}</span></td><td>${escapeHtml(r.Recapito||'—')}</td><td>${statusMarkup(r,i)}</td><td>${url?`<a href="${escapeAttr(url)}" target="_blank" rel="noopener noreferrer">Apri ${icon('external','icon icon-sm')}</a>`:'—'}</td></tr>`}).join('')}</tbody></table></div>`;
 const mobile=`<div class="seg-mobile-list" aria-label="Segnalazioni ricevute">${rows.map(r=>{const i=segnalazioni.indexOf(r),st=r.Stato||'Aperta',state=st==='Chiusa'?'state-chiusa':st==='In gestione'?'state-gestione':'state-aperta',url=safeExternalUrl(r['Link Risposta']);return `<article class="seg-mobile-card ${state}"><div class="seg-mobile-head"><div class="seg-mobile-date-wrap"><div class="seg-mobile-date-icon">${icon('report')}</div><div><div class="seg-mobile-kicker">Segnalazione</div><div class="seg-mobile-date">${escapeHtml(dateLong(r.Data))}</div></div></div>${statusMarkup(r,i)}</div><div class="seg-mobile-body"><div class="seg-mobile-field"><div class="seg-mobile-field-label">Componente</div><div class="seg-mobile-field-value"><span class="badge ${componentClass(r.Componente)}">${escapeHtml(componentDisplay(r.Componente))}</span></div></div><div class="seg-mobile-field"><div class="seg-mobile-field-label">Contatto</div><div class="seg-mobile-field-value">${escapeHtml(r.Recapito||'—')}</div></div><div class="seg-mobile-field"><div class="seg-mobile-field-label">Risposta</div><div class="seg-mobile-field-value">${url?`<a href="${escapeAttr(url)}" target="_blank" rel="noopener noreferrer">Apri risposta ${icon('external','icon icon-sm')}</a>`:'Non disponibile'}</div></div></div></article>`}).join('')}</div>`;
...
function finishIncontriLoad(){loadingInc=false;$('refresh-incontri').disabled=false;$('incontri-list').removeAttribute('aria-busy')}
...
function renderMeetingCard(inc){const date=meetingDate(inc),dp=meetingParts(inc.dataIso||inc.titolo),past=isPast(inc),today=isToday(inc),cls=past?'passato':today?'oggi':'futuro',my=getResponse(currentUser,inc.id),myText=my==='si'?'Hai indicato: disponibile':my==='no'?'Hai indicato: non disponibile':'Non hai ancora risposto',counts=dispCounts(inc.id),pct=counts.total?Math.round(counts.yes/counts.total*100):0,open=!!openPanels[inc.id],seg=inc.segnalazione?parseMeetingSummary(inc.segnalazione):null;
 ...
 const linked=seg?`<div class="inc-linked"><div class="inc-linked-icon">${icon('report')}</div><div><span class="inc-linked-label">Segnalazione collegata</span><div class="inc-linked-details"><span class="inc-linked-main">${escapeHtml(dateBadge(seg.date))}</span>${seg.component?`<span class="inc-linked-meta">${escapeHtml(componentDisplay(seg.component))}</span>`:''}</div></div></div>`:'';
 const response=!past?`<div class="inc-response-row"><div><div class="inc-response-label">La tua disponibilità</div><div class="inc-response-state" id="my-state-${escapeAttr(inc.id)}">${escapeHtml(myText)}</div></div><div class="inc-response-buttons"><button class="btn-yes ${my==='si'?'active':''}" id="btn-yes-${escapeAttr(inc.id)}" type="button" data-action="availability" data-response="si">${icon('check')}Ci sono</button><button class="btn-no ${my==='no'?'active':''}" id="btn-no-${escapeAttr(inc.id)}" type="button" data-action="availability" data-response="no">${icon('x')}Non posso</button></div></div>`:'';
 const details=(seg||true)?`<div class="disp-panel-wrap${open?' open':''}" id="disp-wrap-${escapeAttr(inc.id)}"><div class="disp-panel">${seg?`<div class="seg-info"><div class="seg-info-row">Data: <span>${escapeHtml(dateBadge(seg.date))}</span></div>${seg.component?`<div class="seg-info-row">Componente: <span>${escapeHtml(componentDisplay(seg.component))}</span></div>`:''}${seg.contact?`<div class="seg-info-row">Contatto: <span>${escapeHtml(seg.contact)}</span></div>`:''}</div>`:''}${renderDispPanel(inc.id)}</div></div>`:'';
 return `<article class="inc-card ${cls}" data-meeting-id="${escapeAttr(inc.id)}">${head}<div class="inc-meta-row"><span class="inc-meta-item">${icon('clock','icon icon-sm')}<strong>${escapeHtml(inc.orario||'Orario da definire')}</strong></span>${inc.luogo?`<span class="inc-meta-separator">·</span><span class="inc-meta-item inc-meta-place">${icon('pin','icon icon-sm')}${escapeHtml(inc.luogo)}</span>`:''}</div>${linked}${response}<div class="inc-attendance"><div class="progress-bar"><div class="progress-fill" id="fill-${escapeAttr(inc.id)}" style="width:${pct}%"></div></div><p class="progress-label" id="plabel-${escapeAttr(inc.id)}">${counts.yes} disponibili su ${counts.total}</p><div class="attendance-breakdown" id="breakdown-${escapeAttr(inc.id)}"><span class="yes">${counts.yes} disponibili</span><span class="no">${counts.no} non disponibili</span><span>${counts.wait} in attesa</span></div><button class="btn-small availability-trigger" id="disp-btn-${escapeAttr(inc.id)}" type="button" data-action="toggle-availability" aria-expanded="${open?'true':'false'}"><span class="availability-label">${icon(open?'chevron-up':'chevron-down')}<span>${open?'Nascondi disponibilità':'Vedi disponibilità'}</span></span><span class="availability-count">${counts.yes} sì · ${counts.no} no · ${counts.wait} in attesa</span></button></div>${details}${isCoord?`<div class="inc-admin-actions"><button class="btn-small" type="button" data-action="edit-meeting">${icon('edit')}Modifica</button><button class="btn-danger" type="button" data-action="delete-meeting">${icon('trash')}Elimina</button></div>`:''}</article>`}
...
function openMeetingModal(inc=null){editingId=inc?.id||null;$('modal-title').textContent=inc?'Modifica incontro':'Nuovo incontro';$('modal-data').value=inc?isoDate(meetingDate(inc)):'';$('modal-orario').value=inc?.orario||'';$('modal-luogo').value=inc?.luogo||'';$('modal-segnalazione').value=inc?.segnalazioneId||'';$('modal-error').classList.remove('visible');$('modal-error').textContent='';openDialog('modal-bg','#modal-data')}
