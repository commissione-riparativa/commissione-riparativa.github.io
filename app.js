...
function renderSegnalazioni(){const all=[...segnalazioni].sort((a,b)=>segTimestamp(b.Data)-segTimestamp(a.Data));const counts={all:all.length,'Aperta':all.filter(r=>(r.Stato||'Aperta')==='Aperta').length,'In gestione':all.filter(r=>r.Stato==='In gestione').length,'Chiusa':all.filter(r=>r.Stato==='Chiusa').length};$('stat-total').textContent=counts.all;$('stat-aperte').textContent=counts.Aperta;$('stat-gestione').textContent=counts['In gestione'];$('stat-chiuse').textContent=counts.Chiusa;const rows=currentSegFilter==='all'?all:all.filter(r=>(r.Stato||'Aperta')===currentSegFilter);if(!all.length){$('table-container').innerHTML=emptyState('report','Nessuna segnalazione','Le nuove segnalazioni appariranno qui.');return}if(!rows.length){$('table-container').innerHTML=emptyState('report','Nessuna segnalazione in questa categoria','Scegli un altro filtro per vedere le altre segnalazioni.');return}
 ...
 const mobile=`<div class="seg-mobile-list" aria-label="Segnalazioni ricevute">${rows.map(r=>{const i=segnalazioni.indexOf(r),st=r.Stato||'Aperta',state=st==='Chiusa'?'state-chiusa':st==='In gestione'?'state-gestione':'state-aperta',parts=meetingParts(r.Data),url=safeExternalUrl(r['Link Risposta']),meetingAction=quickMeetingAction(r),deleteAction=deleteSegAction(r),contact=r.Recapito||'';return `<article class="seg-mobile-card seg-mobile-card-compact ${state}"><div class="seg-mobile-row"><div class="seg-mobile-date-stack">${parts?`<span class="seg-mobile-date-day">${escapeHtml(parts.day)}</span><span class="seg-mobile-date-month">${escapeHtml(parts.month.toLowerCase())}</span><span class="seg-mobile-date-year">${escapeHtml(parts.year)}</span>`:`<span class="seg-mobile-date-fallback">Data</span>`}</div><div class="seg-mobile-maincompact"><div class="seg-mobile-mainline"><span class="badge ${componentClass(r.Componente)}">${escapeHtml(componentDisplay(r.Componente))}</span></div>${contact?`<div class="seg-mobile-contact">${escapeHtml(contact)}</div>`:''}<div class="seg-mobile-status-wrap">${statusMarkup(r,i)}</div></div></div><div class="seg-mobile-actions-compact">${url?`<a class="seg-response-link seg-response-link-compact" href="${escapeAttr(url)}" target="_blank" rel="noopener noreferrer">Apri risposta ${icon('external','icon icon-sm')}</a>`:''}${meetingAction}${deleteAction}</div></article>`}).join('')}</div>`;
...
function setMeetingCardOpen(id,open){const card=[...document.querySelectorAll('.inc-card[data-meeting-id]')].find(x=>String(x.dataset.meetingId)===String(id));if(!card)return;const shell=card.querySelector('.inc-details-shell');const btn=card.querySelector('[data-action="toggle-meeting-card"]');const labels=card.querySelectorAll('.inc-toggle-label');const icons=card.querySelectorAll('.inc-toggle-icon');card.classList.toggle('open',open);if(btn)btn.setAttribute('aria-expanded',open?'true':'false');labels.forEach(el=>el.textContent=open?'Nascondi dettagli':'Mostra dettagli');icons.forEach(el=>el.innerHTML=icon(open?'chevron-up':'chevron-down','icon icon-sm'));if(!shell)return;if(open){shell.classList.add('open');shell.style.maxHeight=shell.scrollHeight+'px';setTimeout(()=>{if(openMeetingCardId===id&&shell.classList.contains('open'))shell.style.maxHeight='none'},270)}else{shell.style.maxHeight=(shell.scrollHeight||0)+'px';requestAnimationFrame(()=>{shell.style.maxHeight='0px';shell.classList.remove('open')})}}
...
function renderMeetingCard(inc){const dp=meetingParts(inc.dataIso||inc.titolo),past=isPast(inc),today=isToday(inc),cls=past?'passato':today?'oggi':'futuro',my=getResponse(currentUser,inc.id),myText=my==='si'?'Hai indicato: disponibile':my==='no'?'Hai indicato: non disponibile':'Non hai ancora risposto',counts=dispCounts(inc.id),total=counts.total||0,yesPct=total?Math.round(counts.yes/total*100):0,noPct=total?Math.round(counts.no/total*100):0,waitPct=Math.max(0,100-yesPct-noPct),open=!!openPanels[inc.id],seg=inc.segnalazione?parseMeetingSummary(inc.segnalazione):null,component=seg?.component?componentDisplay(seg.component):'Incontro',componentCls=seg?.component?componentClass(seg.component):'badge-neutral',cardOpen=openMeetingCardId===inc.id;
 ...
 const detailBlock=`<div class="inc-details-shell${cardOpen?' open':''}" id="details-${escapeAttr(inc.id)}"><div class="inc-details-inner">${metaRow}${linked}${response}${attendance}${details}${admin}</div></div>`;
 if(window.innerWidth<=700){return `<article class="inc-card inc-card-mobile ${cls}${cardOpen?' open':''}" data-meeting-id="${escapeAttr(inc.id)}">${mobileSummary}${detailBlock}</article>`}
 return `<article class="inc-card inc-card-desktop ${cls}${cardOpen?' open':''}" data-meeting-id="${escapeAttr(inc.id)}">${desktopSummary}${detailBlock}</article>`}
