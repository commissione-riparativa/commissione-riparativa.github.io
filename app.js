...
function showPage(id){const order=['segnalazioni','incontri','bacheca'],previous=currentPage,direction=Math.sign(order.indexOf(id)-order.indexOf(previous));openMeetingCardId='';openPanels={};if(currentUser&&$('incontri-list'))renderIncontri();document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id==='page-'+id));document.querySelectorAll('.nav-wrap button').forEach(b=>{const active=b.dataset.page===id;b.classList.toggle('active',active);if(active)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current')});currentPage=id;updateNavIndicator(id,true);animatePageEntry($('page-'+id),direction);if(id==='bacheca'){if(!avvisiLoadedOnce)loadAvvisi(false,true);else{renderAvvisi();markAvvisiSeen()}}closeMobileMenu()}
